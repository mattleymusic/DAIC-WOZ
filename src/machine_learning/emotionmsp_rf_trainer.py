#!/usr/bin/env python3
"""
EmotionMSP Chunk-Level Random Forest Trainer with Majority Voting for DAIC-WOZ Dataset

This script trains Random Forest models at the chunk level on EmotionMSP features for all available chunk lengths.
...
Usage:
    python src/machine_learning/emotionmsp_rf_trainer.py
...
def train_emotionmsp_rf_model(train_features, train_labels, dev_features, dev_labels, random_state=42):
    """
    Train a Random Forest model using the predefined train/dev splits on EmotionMSP features.
...
def save_emotionmsp_results(model, scaler, results, chunk_length, overlap, results_dir="data/results"):
    ...
    experiment_id = f"emotionmsp_chunk_rf_{chunk_length}_{overlap}_overlap_{timestamp}"
    ...
    with open(results_path, 'w') as f:
        f.write(f"EmotionMSP Chunk-Level Random Forest Training Results for {chunk_length}_{overlap}_overlap\n")
...
def main():
    """
    Main function to run EmotionMSP Random Forest training for all available chunk lengths.
    Uses predefined train/dev/test splits from meta_info.csv.
    Handles class imbalance and excludes specified participants.
    """
    print("EmotionMSP Random Forest Trainer for DAIC-WOZ Dataset")
    print("=" * 50)
    ...
    try:
        ...
        for i, chunk_length in enumerate(chunk_lengths_to_train):
            ...
            try:
                ...
                print("Loading EmotionMSP chunk features and labels...")
                ...
                print("\nTraining EmotionMSP Random Forest model on train set...")
                model, scaler, dev_predictions = train_emotionmsp_rf_model(
                ...
                if test_results:
                    ...
                    with open(test_results_path, 'w') as f:
                        f.write(f"EmotionMSP Chunk-Level Random Forest Test Results for {chunk_length}_{overlap}_overlap\n")
        ...
        print(f"\n{'='*80}")
        print("EmotionMSP Random Forest training completed for all chunk lengths!")
