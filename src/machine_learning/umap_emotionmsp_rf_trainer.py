#!/usr/bin/env python3
"""
UMAP-emotionMSP Chunk-Level Random Forest Trainer with Majority Voting for DAIC-WOZ Dataset

This script trains Random Forest models at the chunk level on UMAP-transformed emotionMSP features for all available chunk lengths.
...
Usage:
    python src/machine_learning/umap_emotionmsp_rf_trainer.py
...
def load_umap_emotionmsp_chunk_features_and_labels(chunk_length, overlap, data_root="data"):
    """
    Load UMAP-transformed emotionMSP features at chunk level with corresponding participant labels.
    ...
    feature_dir = f"{data_root}/features/umap_emotionmsp/{chunk_length}_{overlap}_overlap"
...
def train_umap_emotionmsp_rf_model(train_features, train_labels, dev_features, dev_labels, random_state=42):
    """
    Train a Random Forest model using the predefined train/dev splits on UMAP-transformed emotionMSP features.
...
def save_umap_emotionmsp_results(model, scaler, results, chunk_length, overlap, results_dir="data/results"):
    """
    ...
    feature_dir = os.path.join(results_dir, "umap_emotionmsp")
    os.makedirs(feature_dir, exist_ok=True)

    # Create timestamped directory for this run
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    experiment_id = f"umap_emotionmsp_chunk_rf_{chunk_length}_{overlap}_overlap_{timestamp}"
    ...
    with open(results_path, 'w') as f:
        f.write(f"UMAP-emotionMSP Chunk-Level Random Forest Training Results for {chunk_length}_{overlap}_overlap\n")
...
def main():
    """
    Main function to run UMAP-emotionMSP Random Forest training for all available chunk lengths.
    Uses predefined train/dev/test splits from meta_info.csv.
    Handles class imbalance and excludes specified participants.
    """
    print("UMAP-emotionMSP Random Forest Trainer for DAIC-WOZ Dataset")
    print("=" * 50)
    ...
    try:
        ...
        for i, chunk_length in enumerate(chunk_lengths_to_train):
            ...
            try:
                ...
                print("Loading UMAP-emotionMSP chunk features and labels...")
                data = load_umap_emotionmsp_chunk_features_and_labels(chunk_length, overlap)
                ...
                print(f"\nLoaded UMAP-emotionMSP features with {len(feature_names)} dimensions")
                ...
                print("\nTraining UMAP-emotionMSP Random Forest model on train set...")
                model, scaler, dev_predictions = train_umap_emotionmsp_rf_model(
                ...
                print("\nSaving results...")
                run_dir = save_umap_emotionmsp_results(model, scaler, dev_results, chunk_length, overlap)

                # Save test results separately if available
                if test_results:
                    test_results_path = os.path.join(run_dir, "test_results.txt")
                    with open(test_results_path, 'w') as f:
                        f.write(f"UMAP-emotionMSP Chunk-Level Random Forest Test Results for {chunk_length}_{overlap}_overlap\n")
        ...
        print(f"\n{'='*80}")
        print("UMAP-emotionMSP Random Forest training completed for all chunk lengths!")
        print(f"Total training time: {total_training_time:.2f} seconds")
        print(f"Average time per chunk length: {total_training_time/len(chunk_lengths_to_train):.2f} seconds")
        print("Check the data/results/umap_emotionmsp/ directory for all saved models and results.")
