#!/usr/bin/env python3
"""
HuBERT Chunk-Level Random Forest Trainer with Majority Voting for DAIC-WOZ Dataset

This script trains Random Forest models at the chunk level on HuBERT features for all available chunk lengths.
...
Usage:
    python src/machine_learning/hubert_rf_trainer.py
...
def train_hubert_rf_model(train_features, train_labels, dev_features, dev_labels, random_state=42):
    """
    Train a Random Forest model using the predefined train/dev splits on HuBERT features.
...
def save_hubert_results(model, scaler, results, chunk_length, overlap, results_dir="data/results"):
    ...
    experiment_id = f"hubert_chunk_rf_{chunk_length}_{overlap}_overlap_{timestamp}"
    ...
    with open(results_path, 'w') as f:
        f.write(f"HuBERT Chunk-Level Random Forest Training Results for {chunk_length}_{overlap}_overlap\n")
...
def main():
    """
    Main function to run HuBERT Random Forest training for all available chunk lengths.
    Uses predefined train/dev/test splits from meta_info.csv.
    Handles class imbalance and excludes specified participants.
    """
    print("HuBERT Random Forest Trainer for DAIC-WOZ Dataset")
    print("=" * 50)
    ...
    try:
        ...
        for i, chunk_length in enumerate(chunk_lengths_to_train):
            ...
            try:
                ...
                print("\nTraining HuBERT Random Forest model on train set...")
                model, scaler, dev_predictions = train_hubert_rf_model(
                ...
                if test_results:
                    ...
                    with open(test_results_path, 'w') as f:
                        f.write(f"HuBERT Chunk-Level Random Forest Test Results for {chunk_length}_{overlap}_overlap\n")
        ...
        print(f"\n{'='*80}")
        print("HuBERT Random Forest training completed for all chunk lengths!")
