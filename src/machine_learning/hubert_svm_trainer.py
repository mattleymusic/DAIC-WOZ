#!/usr/bin/env python3
"""
HuBERT Chunk-Level SVM Trainer with Majority Voting for DAIC-WOZ Dataset

This script trains traditional SVM models at the chunk level on HuBERT features for all available chunk lengths.
...
Usage:
    python src/machine_learning/hubert_svm_trainer.py
...
def load_hubert_chunk_features_and_labels(chunk_length, overlap, data_root="data"):
    """
    Load HuBERT features at chunk level with corresponding participant labels.
    ...
    """
    # Construct the feature directory path for HuBERT features
...
def train_hubert_svm_model(train_features, train_labels, dev_features, dev_labels, random_state=42):
    """
    Train a traditional SVM model using the predefined train/dev splits on HuBERT features.
...
def evaluate_hubert_model(y_true, y_pred, split_name="dev"):
    """
    Evaluate the model performance on HuBERT features.
...
def save_hubert_results(model, scaler, results, chunk_length, overlap, results_dir="data/results"):
    """
    ...
    feature_dir = os.path.join(results_dir, "hubert")
    os.makedirs(feature_dir, exist_ok=True)

    # Create timestamped directory for this run
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    experiment_id = f"hubert_chunk_svm_{chunk_length}_{overlap}_overlap_{timestamp}"
    ...
    with open(results_path, 'w') as f:
        f.write(f"HuBERT Chunk-Level SVM Training Results for {chunk_length}_{overlap}_overlap\n")
...
def main():
    """
    Main function to run HuBERT SVM training for all available chunk lengths.
    Uses predefined train/dev/test splits from meta_info.csv.
    Handles class imbalance and excludes specified participants.
    """
    print("HuBERT SVM Trainer for DAIC-WOZ Dataset")
    print("=" * 50)
    ...
    try:
        ...
        for i, chunk_length in enumerate(chunk_lengths_to_train):
            ...
            try:
                ...
                print("Loading HuBERT chunk features and labels...")
                data = load_hubert_chunk_features_and_labels(chunk_length, overlap)
                ...
                print(f"\nLoaded HuBERT features with {len(feature_names)} dimensions")
                ...
                print("\nTraining HuBERT SVM model on train set...")
                model, scaler, dev_predictions = train_hubert_svm_model(
                ...
                print("\nSaving results...")
                run_dir = save_hubert_results(model, scaler, dev_results, chunk_length, overlap)

                # Save test results separately if available
                if test_results:
                    test_results_path = os.path.join(run_dir, "test_results.txt")
                    with open(test_results_path, 'w') as f:
                        f.write(f"HuBERT Chunk-Level SVM Test Results for {chunk_length}_{overlap}_overlap\n")
        ...
        print(f"\n{'='*80}")
        print("HuBERT SVM training completed for all chunk lengths!")
        print(f"Total training time: {total_training_time:.2f} seconds")
        print(f"Average time per chunk length: {total_training_time/len(chunk_lengths_to_train):.2f} seconds")
        print("Check the data/results/hubert/ directory for all saved models and results.")
