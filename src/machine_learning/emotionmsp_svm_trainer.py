#!/usr/bin/env python3
"""
EmotionMSP Chunk-Level SVM Trainer with Majority Voting for DAIC-WOZ Dataset

This script trains traditional SVM models at the chunk level on EmotionMSP features for all available chunk lengths.
It performs majority voting to aggregate chunk predictions to patient-level predictions.
Handles class imbalance using balanced class weights with default SVM parameters.
...
- Uses default SVM parameters (C=1.0, kernel='rbf', gamma='scale')
- Saves both chunk-level and patient-level results
- Reports performance at both chunk and patient levels

Usage:
    python src/machine_learning/emotionmsp_svm_trainer.py
...
def load_emotionmsp_chunk_features_and_labels(chunk_length, overlap, data_root="data"):
    """
    Load EmotionMSP features at chunk level with corresponding participant labels.
...
def train_emotionmsp_svm_model(train_features, train_labels, dev_features, dev_labels, random_state=42):
    """
    Train a traditional SVM model using the predefined train/dev splits on EmotionMSP features.
    Handles class imbalance using class weights with default SVM parameters.
    ...
    print("Training SVM with default parameters (C=1.0, kernel='rbf', gamma='scale')...")
    ...
    svm_model = SVC(
        ...
        probability=True          # Enable probability estimates for majority voting
    )
    ...
    with tqdm(total=1, desc="SVM Training", unit="model") as pbar:
        pbar.set_postfix({
    ...
    dev_predictions = svm_model.predict(dev_features_scaled)

    return svm_model, scaler, dev_predictions
...
def evaluate_emotionmsp_model(y_true, y_pred, split_name="dev"):
    """
    Evaluate the model performance on EmotionMSP features.
...
def save_emotionmsp_results(model, scaler, results, chunk_length, overlap, results_dir="data/results"):
    """
    ...
    feature_dir = os.path.join(results_dir, "emotionmsp")
    os.makedirs(feature_dir, exist_ok=True)

    # Create timestamped directory for this run
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    experiment_id = f"emotionmsp_chunk_svm_{chunk_length}_{overlap}_overlap_{timestamp}"
    ...
    with open(results_path, 'w') as f:
        f.write(f"EmotionMSP Chunk-Level SVM Training Results for {chunk_length}_{overlap}_overlap\n")
...
def main():
    """
    Main function to run EmotionMSP SVM training for all available chunk lengths.
    Uses predefined train/dev/test splits from meta_info.csv.
    Handles class imbalance and excludes specified participants.
    """
    print("EmotionMSP SVM Trainer for DAIC-WOZ Dataset")
    print("=" * 50)
    ...
    try:
        ...
        for i, chunk_length in enumerate(chunk_lengths_to_train):
            ...
            try:
                ...
                print("Loading EmotionMSP features and labels...")
                data = load_emotionmsp_chunk_features_and_labels(chunk_length, overlap)
                ...
                print(f"\nLoaded EmotionMSP features with {len(feature_names)} dimensions")
                ...
                print("\nTraining EmotionMSP SVM model on train set...")
                model, scaler, dev_predictions = train_emotionmsp_svm_model(
                ...
                print("\nSaving results...")
                run_dir = save_emotionmsp_results(model, scaler, dev_results, chunk_length, overlap)

                # Save test results separately if available
                if test_results:
                    test_results_path = os.path.join(run_dir, "test_results.txt")
                    with open(test_results_path, 'w') as f:
                        f.write(f"EmotionMSP Chunk-Level SVM Test Results for {chunk_length}_{overlap}_overlap\n")
        ...
        print(f"\n{'='*80}")
        print("EmotionMSP SVM training completed for all chunk lengths!")
        print(f"Total training time: {total_training_time:.2f} seconds")
        print(f"Average time per chunk length: {total_training_time/len(chunk_lengths_to_train):.2f} seconds")
        print("Check the data/results/emotionmsp/ directory for all saved models and results.")
